...
import re
from core.query_graphDB import add_new_report
...
def normalize_lines(lines):
    result = []
    for line in lines:
        # Tách theo cả <br> và <br/>
        parts = line.replace("<br/>", "<br>").split("<br>")
        
        # Loại bỏ các phần rỗng và thêm vào mảng kết quả
        for p in parts:
            p = p.strip()
            if p:
                result.append(p)
...
def label_page(lines,labels, num_page, type_report):
    ...
    for l in lines:
        ...
        if ('CÁC THUYẾT MINH ĐÍNH KÈM' in l.strip().upper()) or ('CÁC THUYẾT MINH NÀY' in  l.strip().upper()) :
            continue
    ...
    if num_page == 0 and max < 0.9:
        most_similar_label = "None"
    if num_page < 4 and most_similar_label == 'THUYẾT MINH BÁO CÁO TÀI CHÍNH':
        most_similar_label = 'None'
...
def get_name_report_and_times(pages1):
    prompt = f"""
        ...
        - Hãy trả về các thông tin tôi cần trực tiếp và cách nhau bởi dấu phẩy, KHÔNG giải thích, KHÔNG chú thích,chỉ trả về theo thứ tự như tôi đã yêu đâu ở trên như mấu sau.
            tendoanhnghiep, tenbaocao, thoidiem, linhvuc
    ...
    """
    response = call_api_gemi(prompt,model="gemini-2.5-flash-lite")
    # response = call_gemini_live(prompt)
...
def build_json(pages, names, is_Built = False):
    ...
    for i, page in enumerate(pages):
        ...
        if title == "BÁO CÁO KẾT QUẢ KINH DOANH" or title == "BÁO CÁO KẾT QUẢ HOẠT ĐỘNG" or title == "KẾT QUẢ HOẠT ĐỘNG KINH DOANH":
...
def normalize_report_final(data):
    ...
    # giới thiệu thì thêm vào đầu danh sách
...
def build_graph(file_path = None, is_build = False):
    pages = []
